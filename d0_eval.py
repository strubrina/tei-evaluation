"""
Dimension 0 Evaluation Entry Point.

This script provides command-line interface for running Dimension 0
(XML Well-formedness) evaluation on TEI files.
"""
...
from tei_evaluator import D0Syntactic
from tei_evaluator.reporting import D0Reporter
...
def run_evaluation(xml_directory: Optional[str] = None,
    ...
    if not quiet:
        print("\n=== TEI DIMENSION 0 WELLFORMEDNESS EVALUATION ===\n")

    # Initialize path configuration if needed
    if xml_directory is None or output_directory is None:
        _ = get_path_config()

        if output_directory is None and xml_directory is None:
            output_directory = "evaluation/dimension0"

    # Create evaluator and reporter
    evaluator = D0Syntactic(quiet=quiet)
    reporter = D0Reporter()
    ...
    logger.info("Starting evaluation for directory: %s", xml_directory)
    if not quiet:
    ...
    try:
        # Run evaluation
        results = evaluator.evaluate_directory(xml_directory)
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Dimension 0: XML Well-formedness Evaluation for TEI files",
...
Examples:
  ...
  python d0_eval.py --xml-dir data/output/model1 --output-dir results/d0

  # Evaluate with custom pattern
  python d0_eval.py --xml-dir data/output --pattern "*.tei"
        """
    )
...
def main():
    ...
    # Setup logging (will be configured with proper output dir later)
    setup_logger(
        'tei_evaluator',
        level=log_level,
    ...
    logger = logging.getLogger(__name__)
    logger.info("Starting Dimension 0 evaluation (version %s)", __version__)
    ...
    if args.xml_dir and args.output_dir:
        ...
        log_file = paths.logs_dir / "d0_evaluation.log"
        setup_logger(
            'tei_evaluator',
        ...
        success = run_evaluation(xml_directory, output_directory, quiet=args.quiet)
